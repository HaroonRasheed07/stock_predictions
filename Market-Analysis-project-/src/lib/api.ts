...
export async function fetchAssetSearch(query: string, live: boolean = true, signal?: AbortSignal): Promise<AssetInfo[]> {
  const res = await fetch(`${API_BASE}/api/multi-asset/search?q=${encodeURIComponent(query)}&live=${live}`, { signal });
